...
#             """,
#             movie_id=str(
#                 movie_id),  # movie_id is passed as an int so we need to convert to a string here.
#             identity=movie_id)
...
def insert_movie(id, title, date, lang, budget, popularity, vote_av, runtime):
    with db.session() as session:
        result = session.run(
            """
            CREATE (insertedMovie:Movie {movieId: $id, title: $title, release_date: $date, original_language: $lang, budget: $budget, popularity: $popularity, vote_average: $vote_av, runtime: $runtime})
...
def insert_title(title):
    with db.session() as session:
        result = session.run(
            """
            CREATE (insertedMovie:Movie { title: $title})
            RETURN insertedMovie
            """
            # id=id,
            # title=title,
