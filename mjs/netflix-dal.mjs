...
const getPopularityOfMovie = async movieId => {
      const session = db.session()
...
// }
//title:STRING|release_date:DATE|original_language:STRING|budget:INT|popularity:FLOAT|vote_average:FLOAT|runtime:INT
const insertMovie = async (title, date, language, budget, popularity, vote_average, runtime) => {
  const session = db.session()

  let result
  try {
    result = await session.run(
      `CREATE (insertedMovie:Movie {title: $title, release_date: $date, original_language $language, budget: $budget, popularity: $popularity, vote_average: $vote_average, runtime: $runtime})
       RETURN insertedMovie`,
      { title, date, language, budget, popularity, vote_average, runtime: neo4j.int(year) }
    )
  } finally {
    await session.close()
  }
