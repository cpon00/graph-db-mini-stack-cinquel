import { insertMovie } from "./tmdb-dal.mjs";

if (process.argv.length !== 11) {
  console.log(
    "Usage: add-movie <title> <release_day> <release_month> <release_year> <language> <budget> <popularity> <vote_average> <runtime>"
  );
  process.exit(1);
}
...
try {
  const movieNode = await insertMovie(
    title,
    date,
    language,
    budget,
  ...
  console.error(
    `Sorry, something went wrong. Please ensure that “${year}” is a valid year.`
  );
}
